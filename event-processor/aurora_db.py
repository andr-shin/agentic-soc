...
def get_findings(status_filter=None, severity_filter=None, source_filter=None, limit=100):
    sql = f"SELECT {_FINDING_COLUMNS} FROM findings"
    conditions = []
    params = []
...
def update_finding_status(finding_id, new_status):
    now = datetime.utcnow().isoformat() + 'Z'
    sql = "UPDATE findings SET status = :status, updated_at = :now::timestamptz WHERE finding_id = :fid"
    params = [
